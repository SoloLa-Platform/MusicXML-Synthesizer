...
class Synthesizer():
    ...
    def execute(self):
        ...
        first_downbeat_onset_list = self.get_frist_downbeats(self.raw_downbeats)
        ...
        xml = self.solola_to_xml(solola_format_data)
    ...
    def get_downbeats(self, raw_downbeats):
        ...
        for item in self.raw_downbeats:
            value_list = item.rstrip("\n").split(",")
            if value_list[1] == "1.0":
                downbeat_onset_list.append((float(value_list[0]),float(value_list[1])))
            ...
            while total_index < len(typed_total)-1:
                ...
                if (e1[TYPE_INDEX] == 'no' and e2[TYPE_INDEX] == 'ne') or (e1[TYPE_INDEX] == 'no' and e2[TYPE_INDEX] == 'de'):
                    ...
                    note_dur = round((e2[VALUE_INDEX] -
                                e1[VALUE_INDEX])/(beat_duration/4))
                    if note_dur > 1:
                        tech = tech_and_notes_nparray[e1[VALUE_INDEX] == onset, 3:12].tolist()[0]
                        pitch = tech_and_notes_nparray[e1[VALUE_INDEX] == onset, 0]
    ...
    def plot_timing(self, controlled_count):
        ...
        for data_row in tech_and_notes_nparray:
            if count == controlled_count:
                break;
            plt.plot([data_row[1],data_row[1]+data_row[2]], [data_row[0],data_row[0]])
            count += 1
